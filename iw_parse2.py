...
def write_file(name, data):
    ...
    with open(name, "w") as f:
        f.write(data)
    print("successfully written %d bytes to %s" % (len(data), name), file=sys.stderr)
...
if __name__ == '__main__':
    ...
    # print("json (compacted):\n%s" % (data_json), file=sys.stderr)
    # print("\n\n\n", file=sys.stderr)
    # print(data)
    # print(str(sys.argv))
    
    p = sys.argv[1]
    write_file("%s.csv" % (p), data_csv)
    write_file("%s.json" % (p), data_json)
    write_file("%s.raw.json" % (p), raw)
