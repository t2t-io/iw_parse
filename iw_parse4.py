...
def get_system_wireless_interface():
    import os
    xs = os.listdir('/sys/class/net')
    xs = [ x for x in xs if x.startswith('wlan') ]
    return xs[0] if len(xs) > 0 else 'wlan0'
...
if __name__ == '__main__':
    ...
    import traceback
    try:
        adapter = os.environ['WIRELESS_ADAPTER'] if 'WIRELESS_ADAPTER' in os.environ.keys() else get_system_wireless_interface()
